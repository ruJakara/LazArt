...
- Bot 24/7 hardening pass (2026-03-12):
  - Added bot heartbeat + watchdog in `apps/bot/src/main.py`:
    - heartbeat file refresh every `BOT_HEARTBEAT_INTERVAL_SECONDS` (default `15s`);
    - watchdog thread terminates process on stale heartbeat (`BOT_HEARTBEAT_STALE_AFTER_SECONDS`, default `90s`) so Docker restart policy recovers it.
  - Added container healthcheck script `apps/bot/src/healthcheck.py` (checks token presence + fresh heartbeat).
  - Enabled Docker image health check in `docker/bot/Dockerfile` (`HEALTHCHECK ... python src/healthcheck.py`).
  - Updated local fallback `run-bot-only.bat` to set heartbeat env defaults and write heartbeat into `apps/bot/.tmp`.
  - Added operator script `bot-health.ps1` for one-command runtime diagnostics (`docker engine`, `bot container state/health`, `restart count`, local fallback heartbeat).
  - Updated docs and env template:
    - `.env.example`, `README.md`, `QUICKSTART.md`, `BOT_ONLY.md`, `STATUS.md`.
- Server deploy helper for always-on bot (2026-03-12):
  - Added `deploy-bot.sh` in repo root:
    - validates Docker daemon, compose availability, `.env` presence, and non-placeholder `TELEGRAM_BOT_TOKEN`;
    - runs `docker-compose`/`docker compose` with `docker/docker-compose.prod.yml`;
    - prints service status and bot container health/restart counters;
    - tails bot logs for quick post-deploy verification.
  - Updated docs to use one-command deploy path:
    - `QUICKSTART.md`, `README.md`, `docs/DEPLOYMENT.md`.
